...
if "soft_deleted_files" not in st.session_state: st.session_state.soft_deleted_files = set()
...
L = {
    ...
    "success": "### ✅ Pipeline Render Completed!",
    "preview_title": "🎨 AI Auto-Centering Real-time Matrix Table Grid (Height 拉平並排流)",
    "orig_lbl": "📥 Original Source",
...
if clear_btn_triggered:
    ...
    st.session_state.soft_deleted_files = set()
    st.rerun()
...
if st.session_state.temp_ready:
    ...
    st.stop()
    # 🔒 🔒 🔒 安全隔離防線：內核 else 結構已被物理摧毀，前方一律 0 縮排，絕對不可能報錯 🔒 🔒 🔒
saved = 0
...
for idx, file in enumerate(uploaded_files, 1):
    try:
        ...
        if file_raw_name in st.session_state.soft_deleted_files: continue
        file.seek(0)
...
if st.session_state.temp_ready and st.session_state.master_preview_dict:
    ...
    for orig_file, contents in list(st.session_state.master_preview_dict.items()):
        if orig_file in st.session_state.soft_deleted_files: continue
    ...
    for orig_key, contents in list(st.session_state.master_preview_dict.items()):
        if orig_key in st.session_state.soft_deleted_files or not contents["crops"]: continue
        
        main_col.markdown(f"#### 📁 Asset Folder Source Group: `{orig_key}`")
        import base64
        orig_base64 = base64.b64encode(contents["orig_thumb"]).decode()
        
        html_str = f'<div style="display: flex; align-items: center; background-color: #1a1c23; padding: 12px; border-radius: 8px; margin-bottom: 20px; border: 1px solid #2d3139;">'
        html_str += f'<div style="flex: 0 0 auto; text-align: center; margin-right: 20px; border-right: 2px dashed #3f4452; padding-right: 20px;">'
        html_str += f'<p style="margin:0; font-size:12px; color:#8a90a6; font-weight:bold;">{L["orig_lbl"]}</p>'
        html_str += f'<img src="data:image/jpeg;base64,{orig_base64}" style="height:140px; border-radius:4px; margin-top:6px; object-fit:contain; border:1px solid #3f4452;">'
        html_str += f'</div>'
        
        html_str += f'<div style="flex: 1 1 auto; display: flex; flex-direction: row; overflow-x: auto; white-space: nowrap; gap: 15px; padding-bottom: 5px; scrollbar-width: thin;">'
        for crop_data in contents["crops"]:
            crop_base64 = base64.b64encode(crop_data["thumb_bytes"]).decode()
            html_str += f'<div style="display:inline-block; text-align:left; background:#242933; padding:8px; border-radius:6px; border:1px solid #383f4d;">'
            html_str += f'<img src="data:image/jpeg;base64,{crop_base64}" style="height:140px; border-radius:4px; object-fit:contain;">'
            html_str += f'<p style="margin:4px 0 0 0; font-size:11px; color:#c5c9db; max-width:180px; overflow:hidden; text-overflow:ellipsis;">🎯 {crop_data["img_name"]}</p>'
            html_str += f'</div>'
        html_str += f'</div></div>'
        
        main_col.markdown(html_str, unsafe_allow_html=True)
        
        if main_col.button(f"{L['del_btn']} ── `{orig_key}`", key=f"soft_del_{orig_key}", type="secondary"):
            st.session_state.soft_deleted_files.add(orig_key)
            st.rerun()
