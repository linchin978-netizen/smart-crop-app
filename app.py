...
LANG_MAP["Bahasa Indonesia"] = {
    ...
    * **👑 Paket VAULT RETAIL**: **$49.99** (Dapat **2,000 Kredit** ── **Sangat hemat di bawah $0.025 per gambar!**)
    ...
    "success": "### ✅ Foto AI Selesai! Kredit berhasil dipotong, sebanyak {} aset gambar dibuat!",
            ...
            for idx, file in enumerate(uploaded_files, 1):
                ...
                try:
                    ...
                    for part_idx, (bx, by, bw, bh) in enumerate(valid_boxes, 1):
                        ...
                        with open(os.path.join(temp_out_dir, out_name), "wb") as f_out:
                            f_out.write(final_buf)
                        saved += 1
            ...
            if saved > 0:
                with zipfile.ZipFile(zip_path, "w") as z:
                    for root_dir, _, files in os.walk(temp_out_dir):
                        for f in files:
                            z.write(os.path.join(root_dir, f), f)
                st.session_state.temp_ready = True
                st.rerun()
            else:
                st.error("❌ No image was processed successfully.")
    ...
    if getattr(st.session_state, "temp_ready", False) and os.path.exists(zip_path):
        ...
        if st.download_button(label=L["dl_btn"], data=zip_data, file_name="processed_centered_images.zip", mime="application/zip", use_container_width=True, key="dl_zip_btn_final_gate"):
            if not user_authed:
                ...
                if db and user_uid: 
                    db.collection("users").document(user_uid).update({"credits_total": new_total})
            st.session_state.uploader_key_token += 1
            st.session_state.temp_ready = False
            st.rerun()
